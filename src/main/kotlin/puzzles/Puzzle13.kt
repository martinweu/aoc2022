...
class Puzzle13P1 : Puzzle() {
    ...
    private fun parsePairs(lines: List<String>): MutableList<Pair> {
        ...
        while (lineIndex < lines.size) {
            val a = Expression.fromString(lines[lineIndex++])
            val b = Expression.fromString(lines[lineIndex++])
...
class Puzzle13P2 : Puzzle() {
    override fun solve(lines: List<String>): String {
        ...
        val start = Expression.fromString("[[2]]")
        val end = Expression.fromString("[[6]]")
    ...
    private fun parseExpressions(lines: List<String>): MutableList<Expression> {
        var lineIndex = 0
        val listOfExpressions = mutableListOf<Expression>()
...
data class Pair(val index: Int, val a: Expression, val b: Expression) {
    fun check(): Boolean {
        return a.check(b) <= 0
    }
